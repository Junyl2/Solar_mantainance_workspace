...
export class OntestUtils {
  ...
  syncGenerationGridData(
    ...
    for (let inv = 0; inv < apiData.length; inv++) {
      ...
      const hardware = this.siteHardwares.filter(f => (
      ...
      for (let i = 0; i < chartLabels.length; i++) {
        ...
        if (data.invertorSummary.length > index) {
          if (type != undefined && label == invertorSummary.dateTime) {
            ...
            index++
          } else if (type == undefined && label == format(new Date(invertorSummary.dateTime), dateFormat)) {
            invertorData[inv][i] = invertorSummary[field]
            index++
          } else {
            if (type == undefined && label > format(new Date(invertorSummary.dateTime), dateFormat)) {
              i--;
              index++;
            }
      ...
      if (site && site.siteHardware && site?.siteHardware.length > 0) {
        for (let i = 0; i < site.siteHardware.length; i++) {
          if (site.siteHardware[i].insName == invertorList[inv].insName && site.siteHardware[i].insNum == invertorList[inv].insNum) {
  ...
  syncInvertorGridData(
    ...
    for (let inv = 0; inv < apiData.length; inv++) {
      const hardware = this.siteHardwares.filter(f => (f.insName == apiData[inv].insName && f.insNum == apiData[inv].insNum && f.hardwareType != 'Hardware6'));
      ...
      if (site && site?.siteHardware.length > 0) {
        for (let i = 0; i < site.siteHardware.length; i++) {
          if (site.siteHardware[i].insName == invertorDataList[inv].insName && site.siteHardware[i].insNum == invertorDataList[inv].insNum) {
            invertorName = site.siteHardware[i].name;
            break;
          }
        }
  ...
  syncWeatherGridData(
    ...
    for (let inv = 0; inv < apiData.length; inv++) {
      ...
      if (site && site?.siteHardware.length > 0) {
        for (let i = 0; i < site.siteHardware.length; i++) {
          if (site.siteHardware[i].insName == weatherDataList[inv].insName && site.siteHardware[i].insNum == weatherDataList[inv].insNum) {
            invertorName = site.siteHardware[i].name;
            break;
          }
        }
